...
void open_klient(char *name_of_file,char *nick)
{
	...
	char akcja;		// akcja - wybor uzytkownika (nowy wpis lub polubienie istniejacego)
	...
	if(akcja=='N')
	{
		...
		else
		{
			...
			else	
			{
				...
				read(STDIN_FILENO,shared_memory[temp_slot].shared_data,SIZE_OF_MSG);		//	przekazania wpisanej wiadomosci odrazu do pamieci dzielonej
				shared_memory[0].slot++;																//////	dodanie nowego slotu do pamieci dzielonej
				strcpy(shared_memory[temp_slot].nick,nick);													//	w tym nick, wpis, polubienia, zwiekszenie zajetch slotow o 1
				shared_memory[temp_slot].shared_data[strlen(shared_memory[temp_slot].shared_data)-1]='\0';	//	i odejmuje uzytkownika z osob aktualnie piszacych wpis
				shared_memory[temp_slot].likes=0;															//	
